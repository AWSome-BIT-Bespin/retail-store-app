...
4. UI BFF는 `/checkout/**`, `/proxy/checkout/**`, `/demo/orders`를 WebSession 인증으로 보호한다. Checkout POST는 CSRF token도 필요하며, Catalog와 Cart 경로는 공개로 유지한다.
5. Cart·Checkout customerId는 별도 `SESSIONID` cookie를 사용한다. 로그인 WebSession과 분리되어 있으므로 로그인 전 장바구니는 유지된다.
