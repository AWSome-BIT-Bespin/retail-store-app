...
1. Base와 Head의 `versions.yaml`을 비교하고, Git 공통 조상부터 Head까지의 변경 파일을 계산한다.
...
같은 Git ref의 자동 실행과 수동 실행은 하나의 concurrency group을 사용해 직렬화한다. 새 실행은 진행 중인 실행을 취소하지 않고 대기한다. 이는 워크플로 실행끼리 사전 확인과 게시 사이에서 같은 태그를 경쟁하는 가능성을 줄인다.
